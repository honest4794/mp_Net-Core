...
from lib.tail_codec import write_u32_le
...
# - Poll Core1 frame_done flag 收集統計資料
# - 管理 backlight / comm / seek / source switching
def task_loop(bus):
    # 依賴的服務（由 bus 注入）
    ...
    def _fill_io_hub():
        ...
        if pack is None and (not cache_active) and io_hub.get_fill_level() < io_prefetch:
            ...
            if w is not None:
                ...
                t1 = time.ticks_us()
                read_us2 = time.ticks_diff(t1, t0)
                tail_off = max_jpeg_bytes
    ...
    while True:
        ...
        if bus.shared.pop("core1_frame_done", False):
            dec_us = int(bus.shared.get("core1_dec_us", 0) or 0)
            read_us = int(bus.shared.get("core1_read_us", 0) or 0)
            read_n = int(bus.shared.get("core1_read_n", 0) or 0)
            frame_idx = int(bus.shared.get("core1_frame_idx", 0) or 0)
            bus.shared["src_idx"] = frame_idx
            mp4["frame"] = frame_idx
            sec_read_us += read_us
            n_read_us += read_us
            sec_read_bytes += read_n
            n_read_bytes += read_n
            _stats_on_frame(0, dec_us)
            did_work = True
            if bus.shared.pop("first_frame_done", False):
