...
def init_lcd(dp_config_path="/dp_config.json"):
    ...
    bytes_per_pixel = 3 if str(pixel_format).startswith("RGB888") else 2

    try:
        import lcd_bus

        spi_cfg_list = (bus.shared.get("SPI") or {}).get("list") if hasattr(bus, "shared") else None
        spi_cfg = None
        if isinstance(spi_cfg_list, list):
            for item in spi_cfg_list:
                try:
                    if int(item.get("id")) == spi_id:
                        spi_cfg = item
                        break
                except Exception:
                    continue
        ...
        if not spi_cfg:
            raise ValueError("SPI id={} not found in bus.shared['SPI']['list']".format(spi_id))

        gpio = spi_cfg.get("GPIO") or {}
        sck_pin = int(gpio.get("sck"))
        mosi_pin = int(gpio.get("mosi"))
        miso_v = gpio.get("miso", None)
        miso_pin = -1 if miso_v is None else int(miso_v)
        freq = int(tft_cfg.get("freq") or spi_cfg.get("baudrate") or 10_000_000)
        if miso_v is None:
            tft_spi = lcd_bus.SPIBus(data=(mosi_pin,), clk=sck_pin, freq=freq, host=spi_id)
        else:
            tft_spi = lcd_bus.SPIBus(sck=sck_pin, mosi=mosi_pin, miso=miso_pin, freq=freq, host=spi_id)
        bus.register_service("lcd_spi_bus", tft_spi)
    except Exception:
