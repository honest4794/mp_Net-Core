...
**列表只含 ID + 總時間**（每筆固定 6 bytes，永遠不會撞 8K payload 上限）；名稱等細節用
`MODE_DETAIL_QUERY` 逐個查（見 §2.6）。
...
- `MODE_LIST_QUERY.mode_type` 指定查邊一組：`0`=全部（LED+SERVO 一次過）、`1`=LED、`2`=SERVO；其餘保留。
- `MODE_LIST_RSP.mode_type` **回音** query 嘅組別（0/1/2），Master 可核對回覆對應邊個查詢。
  ⚠️ query 參數嘅 `mode_type=0` 係「查全部」，同模式識別碼嘅 `mode_type=0`（系統模式）語義唔同，唔好混淆。
...
- 需要模式名稱時，對指定模式逐個下 `MODE_DETAIL_QUERY`（0x3107），見 §2.6。
- 以對方 fastLED 而言：LED 組 `storyModes` 約 12 個燈效、SERVO 組 `servoStoryModes` 2 個機械模式；查全部時合併於一次回覆，分組時只回該組。
...
- `(mode_type, mode_id)` 必須在 `MODE_LIST_RSP` 清單內；不在清單 → Slave 回
  `mode_type=0, mode_id=0, name 空`（或忽略，待決）。
- `name` 為 **UTF-8**（可含中文），用協議原生 `str_u16len`（前置 2B LE 長度 + 內容），
  長度上限 65535 bytes、實際受 8K payload 上限約束。
- `total_ms` 回音與 `MODE_LIST_RSP` 一致，Master 不需另外記憶。
- 流程：`MODE_LIST_QUERY`（0x3101）取 ID+時間 → 對需要的模式逐個
  `MODE_DETAIL_QUERY`（0x3107）取名稱等細節 → 湊齊即得每個模式的完整資訊。
...
7. **Broadcast**：`MODE_SET`／`MODE_STOP` 支援廣播（ADDR=0 ／ NC4 `0xFFFF`）；
   廣播時 Slave 不回覆；`MODE_GET`／`MODE_LIST_QUERY`／`MODE_DETAIL_QUERY` 為指定單播，必回覆。
...
| 3 | 亮度（brightness）走哪條通道 | ✅ 已併入 `MODE_SET.brightness`（`0`–`30`，`0xFF`=不設置） | 對方 `BRIGHTNESS` 併入 MODE_SET；舊 WTT 亮度（0–36）棄用 || 4 | 暫停後續播 | 目前一律重頭開始 | 若要續播，`MODE_SET` 需加 `resume_from_ms` 欄位 |
