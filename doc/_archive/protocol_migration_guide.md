...
> 對象：兩邊要讀這份協議的工程師。
> 內容：對方（fastLED master_timer_slave）**每一條指令**變成我們（mp_Net-Core）整合後的哪一條指令、參數怎麼對、架構怎麼理解。
> 基準：`slave/schema/*.json` 為權威定義；細節見 `doc/ota_changelog.md`、`doc/pixel_0x31xx_integration.md`。
> 最後更新：2026-08-18
...
**Q：誰是權威？**
A：`slave/schema/ota.json`、`pixel.json`、`sync.json`。文件（含本份）只是說明，實作以 schema 為準。
...
> 另外：對方「Timer↔Master」的 5-byte UART 幀（`[0xB4][b1][b2][b3][0xFF]`）是**面板通道**，不在本對照內。我方也有相同幀格式，但 bit6/bit7 語義與對方衝突，屬另一條待決線（見 `doc/protocol_merge_comparison.md` §5.1）。
