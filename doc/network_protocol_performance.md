# 網路 + 協議性能測試總結（mp_Net-Core）

> **用途**：記錄 ESP32-P4 / ESP32-S3 的網路吞吐與 NC4 協議解碼性能基準，以及本回合做的性能優化與結論。接手同事讀這份即可了解「現狀、瓶頸、甜蜜點、已做改動、如何復現驗證」，**不需要重跑測試**。
> **對象**：維護網路層、協議層、或要做性能調校的人。
> **最後更新**：2026-08-19
> **基準**：所有數字以本文件為準；細節定義見 `doc/protocol_nc4.md`（協議格式）、`doc/multi_level_buffer.md`（緩衝架構）。
...
| 檔案 | 改動 | 目的 |
|---|---|---|
| `slave/lib/proto.py` | CRC 用 `memoryview` 切片算（省 bytes 複製）；`feed`/`compact` 改 memoryview slice 賦值（memmove，替代 viper 逐 byte）；新增 `MAX_PAYLOAD` / `RX_BUF_SIZE` / `SEND_CAP` 三常量唯一真相源 | 拆幀零多餘複製 + 大小約定單點管理 |
| `slave/tasks/bus_decode.py` | 消費 rx_hub 改用 view 模式（`get_read_view`/`release_read`），取代 `read_into` copy 模式 | 消掉 slot→_read_buf 一層複製 |
| `slave/lib/net_bus.py` | 發送 `_send_all` 加 `SEND_CAP` 分段；接收 buffer 改硬編碼 `RX_BUF_SIZE`（不再讀 config `size`） | 固化 4K 發送硬約束 + 接收 buffer |
| `slave/lib/circuit_bus.py` | 接收 buffer 改硬編碼 `RX_BUF_SIZE`（不再讀 config `size`） | 與 net_bus 對齊 |
...
| `slave/lib/network_manager.py` | 關 WiFi 省電、LAN DHCP 等待、已連指定 SSID 就沿用不重連 | 根治 ETIMEDOUT / 省電延遲 |
...
| 事項 | 狀態 | 說明 |
|---|---|---|
| addr 過濾 | 未做 | header 的 addr 目前是廣播 `0xFFFF`（device ID 未設定）。**搬進生產前**需補「非廣播且 ≠ 自己 ID 就跳過」 |
...
## 7. 如何修改大小上限（唯一真相源 `lib/proto.py`）

三個大小約定全部收斂在 **`slave/lib/proto.py` 開頭**，各司其職、彼此正交：
...
> ⚠️ 這三個是「工程約定」不是協議硬限制。`MAX_PAYLOAD` 的 `LEN` 欄位是 uint16 理論可到 64KB；定 8K 的理由（對端 ESP、無線長幀不可靠、現有指令 ≤2K）見 `doc/protocol_nc4.md` §1。
