...
> 本文件對應**最新 firmware**（`mp_Make-Tools` build 18:33+，含 lcd_bus 效能修復）
> 與 `slave new/` 的 Python 層（bus_adapter / TFT.py / jpeg_player_task）。
> 舊 `doc/lcd.md` 描述的是舊版 API（init/tx_param/tx_color），**已過時**，以本文件為準。
...
MicroPython 層（slave new/）
  jpeg_player_task / tft_dma_bench / tft_pipeline_visual
        ...
        ▼
  lib/TFT.py（ST7789 driver 包裝）
        │  lcd._bus
        ▼
  lib/bus_adapter.py（SpiBusAdapter / I80BusAdapter / I2cBusAdapter / RgbBusAdapter）
...
2. **PSRAM 代價**：v5.5 driver `esp_ptr_dma_capable()` 只認內部 SRAM，PSRAM 一律在 ISR 內同步 copy → **單筆 32KB PSRAM ≈ 2.3ms**（內部 RAM <100us）。這是 fire 12ms/幀的來源（見 PLAN #2）。
...
```python
from lib.buffer_alloc import alloc_fb, Fb
...
2. **PSRAM framebuffer 的 fire 有 12ms ISR copy 代價**（v5.5 driver 不認 PSRAM DMA 直讀）——見 `doc/plan_psram_zero_block.md` 兩方案（暫緩）。
...
（目前 root 與 ext_mod 都停在 async 版 `5e1875b`，一致。）
