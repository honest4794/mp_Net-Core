# heap_caps — DMA 記憶體分配（MicroPython User C Module）

> **用途**：把 ESP-IDF 的 `heap_caps_*` 記憶體配置/查詢能力包成 Python 介面。
> **分類**：使用教學（02_guides）
> **最後更新**：2026-08-16
> **MicroPython 模組名稱**：`heap_caps`
> **典型用途**：SPI 螢幕 DMA buffer、PSRAM（SPIRAM）大緩衝區、依照能力（caps）查詢目前剩餘記憶體。
> **位置**：User C Module（`ext_mod/mp_heap_caps/` 或獨立 repo `mp_heap_caps/`）
...
- 以及其他目標平台提供的 `CAP_*`（如 `CAP_8BIT`、`CAP_32BIT`、`CAP_EXEC`、`CAP_DEFAULT`）
...
把本 repo 當成 user C module，並 include `micropython.cmake`。
...
使用 `micropython.mk` 以 user C module 方式加入。

---

## 相關文件

- `04_lcd_bus.md` — LCD bus 模組（`lcd_bus.py` 統一總線層 + `spi_dma`/`i80_dma`/`rgb_dma` 非同步 DMA）
- `03_notes/02_buffer_architecture.md` — 多級緩衝架構（L0 分配層：`alloc_dma` 統一入口）
