...
MicroPython 上 DMA 加速的 SD 卡管理套件。繞過 FAT/VFS，直接操作 raw sector，提供檔案級別的讀寫 API。

- 模組：`lib.fast_io`
- 相依：`/sd/alloc.json`、`lib.sys_bus`（提供 `sd_raw`）
- 與 `storage_tool.py`（PC 端導入工具）共用相同 managed area 格式
