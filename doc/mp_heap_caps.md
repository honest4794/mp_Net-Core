# mp_heap_caps — DMA-aware heap & async LCD bus for MicroPython on ESP32

MicroPython User C Module 提供：

- **DMA 記憶體分配** (`heap_caps`) — ESP-IDF `heap_caps_*` 的 MicroPython 封裝
- **非同步 SPI/I80/RGB DMA 傳輸** (`spi_dma` / `i80_dma` / `rgb_dma`) — fire-and-forget 隊列式傳輸，不等返回
- **Python LCD 總線層** (`lcd_bus.py`) — 統一 API，自動檢測多線模式 (Quad/Octal)

## 模塊一覽

| 模塊 | 層級 | 功能 |
|---|---|---|
| `heap_caps` | C | DMA/PSRAM 記憶體分配與查詢 |
| `spi_dma` | C | SPI 非同步 DMA (1/2/4/8 線自動檢測) |
| `i80_dma` | C | I80 並行總線非同步 DMA |
| `rgb_dma` | C | RGB 並行總線非同步 DMA (VSYNC 回調) |
| `lcd_bus` | Python | 統一總線 API (`SpiBus`/`I2cBus`/`I80Bus`/`RgbBus`) |

---

## 架構

```
┌──────────────────────────────────────┐
│  Panel Driver (管理 DC/CS 時序)       │
│  dc.low() → bus.write(cmd)           │
│  dc.high() → bus.write(pixels)       │
├──────────────────────────────────────┤
│  lcd_bus.py  (純 Python)             │
│  SpiBus / I2cBus / I80Bus / RgbBus   │
├──────┬──────┬──────┬─────────────────┤
│spi   │(i2c) │i80   │rgb              │
│_dma  │      │_dma  │_dma             │
│.c    │ mach-│.c    │.c               │
│      │ ine  │      │                 │
│      │.I2C) │      │                 │
├──────┴──────┴──────┴─────────────────┤
│  ESP-IDF GDMA 硬體                   │
│  queue_trans → DMA → get_trans_result│
└──────────────────────────────────────┘
```

### 設計原則

- **DC/CS 不由總線管理** — 交給上層 panel driver
- **data tuple 自動決定線數** — `data=(mosi,)` → 1線, `data=(d0..d3)` → 4線 (Quad), `data=(d0..d7)` → 8線 (Octal)
- **write / readinto / write_readinto 全部非同步** — 返回 `trans_id`，用 `wait()` 查完成
- **隊列深度**: SPI 4級 / I80 4級 / RGB 2級
- **GC 安全**: `ref_bufs[]` 持有 buffer 引用，`wait()` 時釋放

---

## heap_caps — DMA 記憶體分配

`import heap_caps`

### 分配

```python
buf = heap_caps.malloc(320 * 240 * 2, heap_caps.CAP_DMA)       # DMA 可用
buf = heap_caps.malloc(320 * 240 * 2, heap_caps.CAP_SPIRAM)    # PSRAM
buf = heap_caps.malloc(1024, heap_caps.CAP_INTERNAL | heap_caps.CAP_DMA)  # 組合
```

所有分配函數返回 writable `memoryview`，失敗返回 `None`。

| 函數 | 說明 |
|---|---|
| `malloc(size, caps)` | 分配 (zero-initialized) |
| `calloc(count, size, caps)` | 分配並清零 |
| `aligned_alloc(alignment, size, caps)` | 對齊分配 (ESP32 only) |
| `aligned_calloc(alignment, count, size, caps)` | 對齊分配並清零 (ESP32 only) |
| `realloc(buf, size, caps)` | 重新分配 |
| `free(buf)` | 釋放。`buf` 可以是單個 `memoryview` 或 list |

### 查詢

```python
heap_caps.get_free_size(heap_caps.CAP_DMA)
heap_caps.get_total_size(heap_caps.CAP_SPIRAM)
heap_caps.get_largest_free_block(heap_caps.CAP_DMA)  # DMA 需連續區塊
```

### Capability 常數

`CAP_DMA`, `CAP_INTERNAL`, `CAP_SPIRAM`, `CAP_8BIT`, `CAP_32BIT`, `CAP_EXEC`, `CAP_DEFAULT`, 等。

### 安全注意

- `heap_caps.free()` 只能釋放本模塊分配的 buffer，不要傳入 `bytearray` 或其他來源的 memoryview
- 不要傳入 slice (如 `buf[16:]`) — slice 指向偏移地址，釋放會出錯
- buffer 不受 MicroPython GC 管理，必須手動釋放

---

## spi_dma — 非同步 SPI DMA

`import spi_dma`

### 初始化

```python
# 1 線 標準 SPI
spi_dma.init(data=(35,), clk=36, freq=40_000_000)

# 4 線 Quad SPI  (auto-detect from data length)
spi_dma.init(data=(35, 36, 37, 38), clk=39, freq=80_000_000)

# 8 線 Octal SPI
spi_dma.init(data=(d0, d1, d2, d3, d4, d5, d6, d7), clk=..., freq=80_000_000)
```

### API

| 方法 | 非同步 | 返回 | 說明 |
|---|---|---|---|
| `init(data, clk, freq, host)` | — | — | 初始化 SPI 總線 |
| `write(buf)` | ✅ | `trans_id` | fire-and-forget 寫入 |
| `readinto(buf, write_val=0)` | ✅ | `trans_id` | 非同步讀取 |
| `write_readinto(wbuf, rbuf)` | ✅ | `trans_id` | 全雙工非同步 |
| `is_busy()` | — | `bool` | 隊列是否有未完成傳輸 |
| `pending()` | — | `int` | 隊列中等待數 |
| `wait(trans_id, timeout_ms=-1)` | 阻塞 | `bool` | 等待特定傳輸完成 |
| `wait_all(timeout_ms=-1)` | 阻塞 | — | 等待全部完成 + 主動 GC |
| `lane_count()` | — | `int` | 數據線數量 |
| `deinit()` | — | — | 釋放資源 |

### 使用模式

```python
buf = heap_caps.malloc(320 * 240 * 2, heap_caps.CAP_DMA)

tid = spi_dma.write(buf)          # 發射，不等返回
# CPU 離開做其他事...
if spi_dma.is_busy():             # 回來問是否完成
    spi_dma.wait(tid)             # 阻塞等特定傳輸完成

spi_dma.deinit()
heap_caps.free(buf)
```

### 多線自動檢測

`data` tuple 長度決定 SPI 傳輸模式：

| data 長度 | 模式 | SPI_TRANS_MODE |
|---|---|---|
| 1 | 標準 SPI | (default) |
| 2 | Dual SPI | `SPI_TRANS_MODE_DIO` |
| 4 | Quad SPI | `SPI_TRANS_MODE_QIO` |
| 8 | Octal SPI | `SPI_TRANS_MODE_OCT` |

### readinto 原理

`readinto` 用一個預分配的 32KB `zero_buf` 當 tx_buffer (共用，只讀不寫)。多個排隊的讀事務安全共存 — 每個事務讀相同的零值到 MOSI，同時從 MISO 捕獲數據到各自的 rx buffer。rx buffer 被 `ref_bufs[]` pin 住直到 `wait()` 釋放。

---

## i80_dma — 非同步 I80 並行總線 DMA

`import i80_dma`

僅在 ESP32-S3/P4 (支援 `SOC_LCD_I80_SUPPORTED`) 上可用。

### 初始化

```python
# 8-bit I80
i80_dma.init(data=(d0, d1, d2, d3, d4, d5, d6, d7), wr=10, cs=11, freq=10_000_000)

# 16-bit I80
i80_dma.init(data=(d0, d1, ..., d15), wr=10, freq=10_000_000)
```

### API

與 `spi_dma` 一致：`write`, `is_busy`, `pending`, `wait`, `wait_all`, `lane_count`, `deinit`。

不支援 `readinto` / `write_readinto` — I80 總線是單向輸出。

完成信號來自 ESP-IDF 的 `on_color_trans_done` 回調。

---

## rgb_dma — 非同步 RGB 並行總線 DMA

`import rgb_dma`

僅在 ESP32-S3/P4 (支援 `SOC_LCD_RGB_SUPPORTED`) 上可用。

### 初始化

```python
rgb_dma.init(
    data=(d0, d1, d2, d3, d4, d5, d6, d7),
    hsync=12, vsync=13, de=14, pclk=15,
    width=480, height=272, freq=9_000_000,
    hsync_front_porch=2, hsync_back_porch=40, hsync_pulse_width=41,
    vsync_front_porch=2, vsync_back_porch=10, vsync_pulse_width=10,
)
```

### API

| 方法 | 非同步 | 返回 | 說明 |
|---|---|---|---|
| `write(buf, x, y, w, h)` | ✅ | `trans_id` | 寫入像素區域 (預設全螢幕) |
| `is_busy` / `pending` / `wait` / `wait_all` | — | — | 同上 |
| `lane_count` / `deinit` | — | — | — |

不支援 `readinto` / `write_readinto`。

完成信號來自 VSYNC 中斷回調。隊列深度為 2 (雙緩衝由 VSYNC 速律限制)。

---

## lcd_bus.py — Python 統一總線層

```python
from lcd_bus import SpiBus, I2cBus, I80Bus, RgbBus
```

### 四種總線 — 統一 API

```python
# SPI: data=(mosi,), clk
bus = SpiBus(data=(35,), clk=36, freq=40_000_000)

# I2C: data=(sda_pin,), clk=scl_pin, addr
bus = I2cBus(data=(sda,), clk=scl, addr=0x3C, freq=400_000)

# I80: data=(d0..d7), wr
bus = I80Bus(data=(0,1,2,3,4,5,6,7), wr=10)

# RGB: data=(d0..d7), hsync, vsync, de, pclk, width, height
bus = RgbBus(
    data=(0,1,2,3,4,5,6,7),
    hsync=12, vsync=13, de=14, pclk=15,
    width=480, height=272,
)
```

### 統一方法

| 方法 | SPI | I2C | I80 | RGB |
|---|---|---|---|---|
| `write(buf)` | ✅ | ✅ | ✅ | ✅ |
| `readinto(buf)` | ✅ | ✅ | ❌ | ❌ |
| `write_readinto(w, r)` | ✅ | ✅ | ❌ | ❌ |
| `is_busy()` | ✅ | ✅ | ✅ | ✅ |
| `pending()` | ✅ | ✅ | ✅ | ✅ |
| `wait(tid)` | ✅ | ✅ | ✅ | ✅ |
| `wait_all()` | ✅ | ✅ | ✅ | ✅ |
| `lane_count` | ✅ | ✅ | ✅ | ✅ |
| `deinit()` | ✅ | ✅ | ✅ | ✅ |

### 使用模式 (雙緩衝)

```python
bus = SpiBus(data=(35,), clk=36, freq=40_000_000)
buf_a = heap_caps.malloc(FRAME_SIZE, heap_caps.CAP_DMA)
buf_b = heap_caps.malloc(FRAME_SIZE, heap_caps.CAP_DMA)

while True:
    render(buf_a)
    bus.write(buf_a)               # 發射 A
    render(buf_b)
    bus.write(buf_b)               # 發射 B
    if bus.pending() >= 2:         # 回來看看排隊情況
        bus.wait_all()

bus.deinit()
heap_caps.free([buf_a, buf_b])
```

---

## Panel Driver 範例 (ST7789 + SpiBus)

```python
from machine import Pin
from lcd_bus import SpiBus
import heap_caps

dc = Pin(37, Pin.OUT)     # DC 由上層管理
cs = Pin(38, Pin.OUT)

bus = SpiBus(data=(35,), clk=36, freq=40_000_000)

cs.low()
dc.low()
bus.write(b'\x11')        # SLPOUT
bus.wait_all()

dc.high()
fb = heap_caps.malloc(240 * 320 * 2, heap_caps.CAP_DMA)
bus.write(fb)

bus.deinit()
heap_caps.free(fb)
```

---

## 構建

### CMake (ESP32 / ESP-IDF)

`micropython.cmake` 已包含所有源碼：

- `esp32_src/heap_caps.c`
- `esp32_src/spi_dma.c`
- `esp32_src/i80_dma.c`
- `esp32_src/rgb_dma.c`

會自動獲取 `esp_lcd` 的 include 路徑。

### 非 ESP32 平台

僅 `heap_caps` 模塊可用 (`common_src/heap_caps.c` — 功能有限)。

---

## 注意事項

- **DC/CS** 不由總線管理，需上層自行操作 GPIO
- **`heap_caps.free()`** 必須手動調用，不受 GC 管理
- **I2C** 無 DMA 支援，傳輸為阻塞模式；其餘三種總線均支援非同步 DMA
- **RGB** `write()` 的 `x, y, w, h` 預設為 `(0, 0, panel_w, panel_h)`
- **完成時間**: SPI 可用 `wait()` 阻塞等待，I80/RGB 使用忙等輪詢 `done_flags[]`
- **I80/RGB** 僅 ESP32-S3/P4 可用；其他晶片 import 會得到空模塊

---

## 快速測試 + 速度測試

[test_bus.py](file:///test_bus.py) 提供完整功能驗證與速度測試，涵蓋所有總線類型：

```python
import test_bus
test_bus.run_all()
```

### 功能測試

| # | 測試 | 項目 |
|---|---|---|
| 1 | `heap_caps` | 分配/釋放、capability 常數、堆統計 |
| 2 | `SpiBus` | 寫入、4級排隊、隊列滿防護、deinit |
| 3 | SPI 多線 | 1/2/4/8 線自動檢測 |
| 4 | `I2cBus` | 初始化、寫入 |
| 5 | `I80Bus` | 排隊、deinit |
| 6 | `RgbBus` | 全螢幕/區域寫入 |
| 7 | 壓力 | init → write → deinit 循環 3 次 |

### 速度測試 (測試 8)

測量 `write()` 發射時間 + `wait()` 完成時間 + 吞吐量：

| 測試項 | 內容 |
|---|---|
| 8a | SPI 1線 40MHz — 256B/1KB/4KB/16KB/32KB |
| 8b | SPI 1線 — 10/20/40/60/80MHz (32KB) |
| 8c | SPI Quad 4線 40MHz — 4KB/16KB/32KB |
| 8d | SPI Quad 4線 80MHz — 32KB |
| 8e | SPI Octal 8線 40MHz — 32KB |
| 8f | I80 8-bit 10MHz — 16KB/32KB |
| 8g | RGB 8-bit 9MHz — 全螢幕 |
| 8h | 速度摘要 — 各總線 32KB 吞吐量對比 |

輸出格式：

```
  test         │   size │         fire (queue) │          wait (done) │                    total
  ─────────────┼────────┼───────────────────────┼──────────────────────┼────────────────────────
  32KB 1線 40M │  32768B │     42us   763 MB/s │   4500us   7282 KB/s │   4542us   7215 KB/s
  32KB 4線 40M │  32768B │     38us   862 MB/s │   1150us  28494 KB/s │   1188us  27578 KB/s
  32KB 4線 80M │  32768B │     35us   936 MB/s │    590us  55539 KB/s │    625us  52429 KB/s
```

所有測試僅使用 `write()`，不涉及讀取操作。

## License

MIT
