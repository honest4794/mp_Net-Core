#!/usr/bin/env python3
"""Extract a rough LVGL migration inventory from an HTML file.

This is intentionally conservative: it does not try to compile HTML/CSS into LVGL.
It identifies likely controls, labels, tabs, cards, and scroll sections so the
agent can produce a firmware-friendly LVGL plan.
"""
...
def lvgl_guess(node: Node) -> str:
    ...
    if tag == "input" and input_type in {"checkbox", "radio"}:
        return "lv_checkbox"
    if tag == "input" and input_type in {"color"}:
    ...
    if "wheel" in marker and ("color" in marker or "picker" in marker):
        return "lv_colorwheel"
    if tag in {"nav"} or "tab" in classes:
        return "lv_btn row"
...
def extract_inventory(path: Path) -> dict[str, object]:
    ...
    for node in walk(parser.root):
        ...
        if category == "text" and not node.text:
            continue
        if category == "text" and len(node.text) < 2:
            continue
        items.append(
            {
                "tag": node.tag,
...
def emit_markdown(inventory: dict[str, object]) -> str:
    lines = [
        f"# LVGL Inventory: `{inventory['source']}`",
        "",
        "## Summary",
        "",
    ]
    ...
    for item in items:
        ...
        lines.append(
            f"| {item['category']} | `{html_name}` | {item['label']} | `{item['lvgl']}` |"
        )
    lines.extend(
        [
            "",
