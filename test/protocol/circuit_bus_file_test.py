...
def run_circuit_bus_interactive(
    ...
    while True:
        print("")
        print("1) MP4 控制  2) 檔案傳輸  3) Wi-Fi 開關  4) Web Server 開關  5) 任意 CMD  0) 離開")
        cmd = _input_line("> ")
        cmd = "" if cmd is None else str(cmd).strip().lower()
        ...
        if cmd in ("1", "mp4"):
            run_mp4_interactive(uart_id=uart_id, baudrate=baudrate, tx=tx, rx=rx, debug=debug)
            continue
        ...
        if cmd in ("3", "wifi"):
            ...
            uart = _open_uart_with_fallback(uart_id, baudrate, tx, rx)
