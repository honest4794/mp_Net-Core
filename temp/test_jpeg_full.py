...
class DeepBufferPipeline:
    def __init__(self, bus, dc, w, h, bpp, jpeg_frames,
        ...
        self._interval_us = (1000 // target_fps) * 1000 if target_fps > 0 else 0
    ...
    def _tick(self):
        did_output = self._output_try()
        t_decode = self._decode_try()
        if self._pending_tids:
            for tid in self._pending_tids:
                self._bus.wait(tid)
            self._pending_tids = []
            if self._sending_idx >= 0:
                self._free.append(self._sending_idx)
                self._sending_idx = -1
        if did_output:
            now = time.ticks_us()
            if self._last_output_us:
                gap = time.ticks_diff(now, self._last_output_us)
                self._tracker.record(t_decode, 0, gap)
            self._last_output_us = now
            self._count += 1
            if self._count == 1:
                self._tracker.reset()
        if did_output and self._count % 30 == 0:
            self._log_status(t_decode)

    def _output_try(self):
        if self._interval_us > 0:
            now = time.ticks_us()
            if time.ticks_diff(now, self._next_tick_us) > 200000:
                self._next_tick_us = now
            if time.ticks_diff(now, self._next_tick_us) < 0:
                return False
            self._next_tick_us += self._interval_us
    ...
    def _decode_try(self):
        if not self._free:
            return 0
        if self._interval_us > 0:
            pass
        else:
            water_pct = len(self._ready) * 100 // max(self._pool_depth, 1)
            if water_pct >= 50:
                return 0
    ...
    def _log_status(self, t_decode):
        now_ms = time.ticks_ms()
        self._last_log_ms = now_ms
        total_dt = time.ticks_diff(now_ms, self._run_start_ms) if self._run_start_ms else 0
        fps = self._count * 1000.0 / total_dt if total_dt else 0
        water_pct = len(self._ready) * 100 // max(self._pool_depth, 1)
        empty_flag = " E{}".format(self._pool_empty_events) if self._pool_empty_events else ""
        print("  frame={} fps={:.1f} | pool={}/{} ({}%) | d={}us{}".format(
            self._count, fps,
            len(self._ready), self._pool_depth, water_pct,
            t_decode, empty_flag))
...
def run_deep_pipeline(quality=80, pool_depth=8, target_fps=0,
    ...
    gc.collect(); time.sleep_ms(50); gc.collect()
    try:
        import heap_caps
        heap_caps.reset()
        print("  heap_caps.reset()")
        for name, caps in [
            ("SPIRAM   ", heap_caps.CAP_SPIRAM),
            ("DMA      ", heap_caps.CAP_DMA),
            ("INTERNAL ", heap_caps.CAP_INTERNAL),
...
def run_deep_compare(quality=80, n_frames=20, max_display=100):
    ...
    depths = [2, 4, 8, 12, 16]
    for depth in depths:
        gc.collect(); time.sleep_ms(200); gc.collect()
        try:
            import heap_caps
            heap_caps.reset()
        except Exception:
